                            ...
                            <div class="table-responsive scrollbar">
                                <table class="table mb-0">
                                    ...
                                    <tbody id="bulk-select-body">
                                        @forelse ($intro as $items)
                                            <tr>
                                                <th class="align-middle">{{ $loop->index + 1 }}</th>
                                                <th class="align-middle">{{ $items->icon }}</th>
                                                <td class="align-middle">{{ $items->name }}</td>
                                                <td class="align-middle">{{ $items->descripsion }}</td>
                                                <td class="align-middle">
                                                    <div>
                                                        <button class="btn p-0" type="button" 
                                                            data-bs-placement="top" title="Edit" type="button"
                                                            data-bs-toggle="modal" data-bs-target="#edit-modal{{$items->id }}">
                                                             <span class="text-500 fas fa-edit"></span></button>

                                                        <button class="btn p-0" type="button"  
                                                        data-bs-toggle="modal" data-bs-target="#delete-modal{{$items->id }}"
                                            ...
                                            {{-- Intro Edit Modal --}}
                                            @include('page.backend.Intro-Section.modal-include.edit')
                                            @include('page.backend.Intro-Section.modal-include.delete')
                                        @empty
                                            <tr>
                                ...
                                {{-- Insert New Modal --}}
                                @include('page.backend.Intro-Section.modal-include.add')
