        ...
        <div class="card-body bg-light">
            <div class="tab-content">
                <div class="tab-pane preview-tab-pane active" role="tabpanel"
                    ...
                    <form action="{{ route('Hero-Icon-Section-updating') }}"  method="POST" >
                        @csrf
                        @php
                            $Hero_Icon = ['Icon-1' , 'Icon-2' , 'Icon-3' , 'Icon-4']; 
                        @endphp
                        {{-- Hero_Icon --}}
                        @foreach ($Hero_Icon as $items )
                        <div class="row mb-3">
                            <label class="col-sm-2 col-form-label" for="{{ $items }}">{{ $items }}</label>
                            <div class="col-sm-10">
                                <input class="form-control" id="{{ $items }}" name="{{ $items }}" type="text"
                                    value="{{ old($items, $HeroIcon[$items]) }}" />
                            </div>
                        </div>
                        @endforeach
