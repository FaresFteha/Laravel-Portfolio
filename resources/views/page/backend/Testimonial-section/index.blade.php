                    ...
                    <div class="card shadow-none">
                        <div class="card-body p-0 pb-3">
                            ...
                            <div class="table-responsive scrollbar">
                                <table class="table mb-0">
                                    <thead class="text-black bg-200">
                                        <tr>
                                            ...
                                            <th class="align-middle">Reviw</th>
                                            <th class="align-middle">Name</th>
                                            <th class="align-middle">Title Job</th>
                                            <th class="align-middle white-space-nowrap pe-3">Action</th>
                                        </tr>
                                    </thead>
                                    <tbody id="bulk-select-body">
                                        @forelse ($testimonials as $items)
                                            <tr>
                                                <th class="align-middle">{{ $loop->index + 1 }}</th>
                                                <th class="align-middle">
                                                    @if ($items->photo)
                                                        <img src="{{ asset('Attachments/Testimonial_Photo/' . $items->photo) }}"
                                                            width="60" height="60" alt="Testimonial-Photo">
                                                    @endif
                                                </th>
                                                <th class="align-middle">{{ $items->review }}</th>
                                                <td class="align-middle">{{ $items->name }}</td>
                                                <td class="align-middle">{{ $items->title_job }}</td>
                                            ...
                                            {{-- Intro Edit Modal --}}
                                            @include('page.backend.Testimonial-section.modal-include.edit')
                                            @include('page.backend.Testimonial-section.modal-include.delete')
                                        @empty
                                            <tr>
                                                <td class="alert-danger text-center" colspan="7">No Data in this Table</td>
                                            </tr>
                                        @endforelse
                                ...
                                {{-- Insert New Modal --}}
                                @include('page.backend.Testimonial-section.modal-include.add')
