        ...
        <div class="card-header">
            <div class="row flex-between-end">
                <div class="col-auto align-self-center">
                    <h5 class="mb-0" data-anchor="data-anchor">Update</h5>
                </div>
            </div>
        </div>
        <div class="card-body bg-light">
            <div class="tab-content">
                <div class="tab-pane preview-tab-pane active" role="tabpanel"
                    ...
                    <form method="POST" action="{{ route('Blog.update' , 'test') }}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="text" id="title" name="title"
                                value="{{ old('title', $blogs->title) }}" class="@error('title') is-invalid @enderror" />
                            @error('title')
                        ...
                        <div class="mb-3">
                            ...
                            <textarea class="form-control" id="descripsion" name="descripsion" class="@error('descripsion') is-invalid @enderror">{{ old('descripsion', $blogs->descripsion) }}</textarea>
                            @error('descripsion')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
                            ...
                            <select class="form-control" id="organizerMultiple" name="category_id"
                                ...
                                @foreach ($Category as $category)
                                    <option value="{{ $category->id }}"
                                        {{ $category->id == $category->id ? 'selected' : '' }}>{{ $category->name }}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                                <div class="alert alert-danger">{{ $message }}</div>
                        ...
                        <div class="mb-3">
                            ...
                            <input class="form-control" type="text" id="coade_use" name="coade_use"
                                value="{{ old('coade_use', $blogs->coade_use) }}"class="@error('coade_use') is-invalid @enderror" />
                            @error('coade_use')
                                <div class="alert alert-danger">{{ $message }}</div>
                            @enderror
                        </div>
